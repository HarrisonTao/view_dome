...
public class MQUILImageLoader extends MQImageLoader {
    ...
    @Override
    protected void displayImage(Activity activity, ImageView imageView, Uri uri, int loadingResId, int failResId, int width, int height, final MQDisplayImageListener displayImageListener) {
        DisplayImageOptions options = new DisplayImageOptions.Builder()
                .showImageOnLoading(loadingResId)
                .showImageOnFail(failResId)
                .cacheInMemory(true)
        ...
        ImageLoader.getInstance().displayImage(uri.toString(), new ImageViewAware(imageView), options, imageSize, new SimpleImageLoadingListener() {
            @Override
            public void onLoadingComplete(String imageUri, View view, Bitmap loadedImage) {
