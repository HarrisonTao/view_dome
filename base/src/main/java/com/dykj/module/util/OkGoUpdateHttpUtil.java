    ...
    public void download(@NonNull String url, @NonNull String path, @NonNull String fileName, @NonNull final HttpManager.FileCallback callback) {
        OkGo.<File>get(url).execute(new com.lzy.okgo.callback.FileCallback(path, fileName) {
            ...
            public void downloadProgress(Progress progress) {
                ...
                Log.e("jindu=", ((float) progress.currentSize) / (1024 * 1024 * ((float) Flag.APP_SIZE)) + "");
                callback.onProgress(((float) progress.currentSize) / (1024 * 1024 * ((float) Flag.APP_SIZE)), (long) Flag.APP_SIZE);
            }
        });
    }
}