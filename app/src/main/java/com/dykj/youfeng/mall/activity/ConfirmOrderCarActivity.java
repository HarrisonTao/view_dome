...
import com.dykj.youfeng.mall.adapter.CarConfirmAdapter;
 ...
 * @describe: 确认订单页面
 */
public class ConfirmOrderCarActivity extends BaseActivity {
    ...
    private CarConfirmAdapter adapter;
    ...
    private void changPoint() {
        ...
        } else if (Float.parseFloat(etPoint.getText().toString()) > allMoney) {
            DyToast.getInstance().error("最多可用积分" + allMoney);
            etPoint.setText(ArithUtil.formatIntNum(allMoney));
        }
        float point = 0;
        if (!StringUtils.isEmpty(etPoint.getText().toString())) {
    ...
    private void getOrderInfo() {
        ...
        HttpFactory.getMallInstance().orderInfoCar(params)
                .compose(HttpObserver.schedulers(this))
                .as(HttpObserver.life(this))
                .subscribe(new HttpObserver<>(new HttpListener<HttpResponseData<List<JsonObject>>>() {
                    @Override
                    public void success(HttpResponseData<List<JsonObject>> data) {
                        if ("9999".equals(data.status)) {
                            String json = data.getData().get(0).toString();
                            ConfirmOrderCarBean confirmOrderBean = new Gson().fromJson(json, ConfirmOrderCarBean.class);
                            if (data.getData().get(0).get("address_info").toString().length() > 4) {
                                ConfirmOrderCarBean.AddressInfoBean addressBean = new Gson().fromJson(data.getData().get(0).get("address_info").toString(), ConfirmOrderCarBean.AddressInfoBean.class);
    ...
    private void setData(ConfirmOrderCarBean s) {
        ...
        num = 0;
        float usePointNum = 0;
        for (int i = 0; i < s.getInfo().car_list.size(); i++) {
            num = num + Integer.parseInt(s.getInfo().car_list.get(i).goods_num);
            allMoney = allMoney + Integer.parseInt(s.getInfo().car_list.get(i).goods_num) * Float.parseFloat(s.getInfo().car_list.get(i).goods_price);
            usePointNum = usePointNum + Integer.parseInt(s.getInfo().car_list.get(i).goods_num) * Float.parseFloat(s.getInfo().car_list.get(i).goods_price) * Float.parseFloat(s.getInfo().car_list.get(i).goods_points) / 100;
        }
        adapter = new CarConfirmAdapter(R.layout.item_confirm_car, s.getInfo().car_list);
        ...
        if (canUsePoint > 0) {
            if (canUsePoint > usePointNum) {
                canUsePoint = usePointNum;
            }
            usePoint = true;
    ...
    private void getExpMoney() {
        ...
        Map<String, String> params = (Map<String, String>) getIntentData();
        String carId=params.get("cardid");
        HttpFactory.getMallInstance().getFreight(addressId,num+"","",carId)
                .compose(HttpObserver.schedulers(this))
                .as(HttpObserver.life(this))
...
//                getPayType();
    ...
    private void commit() {
        ...
        String mToken = MMKV.defaultMMKV().decodeString(AppCacheInfo.mTokent, "");
        params.put("token", mToken);
        params.put("carid", params.get("cardid"));
//        params.put("freight_id_type", transId + "_" + expType);
        ...
        HttpFactory.getMallInstance().submitOrderCar(params)
                .compose(HttpObserver.schedulers(this))
                .as(HttpObserver.life(this))
                    ...
                    public void success(HttpResponseData<PayWxBean> data) {
                        if ("9999".equals(data.status)) {
                            ...
                            if (null != payWxBean) {
                                ...
                                IWXAPI api = WXAPIFactory.createWXAPI(ConfirmOrderCarActivity.this, appId);
                    ...
                    public void success(HttpResponseData<String> data) {
                        if ("9999".equals(data.status)) {
                            Runnable payRunnable = () -> {
                                PayTask alipay = new PayTask(ConfirmOrderCarActivity.this);
...
//            requestUpInfo();
            startAct(PaySuccessActivity.class, tvMoney.getText().toString());
            DyToast.getInstance().success("支付成功");
            ConfirmOrderCarActivity.this.setResult(1);
            finish();
        }
    }
